<template>
    <div class="flex flex-col lg:flex-row pt-6 lg:pt-12 px-2 lg:px-10w h-full">
        <div class="flex flex-col justify-center items-start slide-in-bottom">
            ...
            <h1 class="text-5xl font-extrabold text-transparent sm:text-6xl my-3 bg-clip-text bg-gradient-to-r from-green-300 via-blue-500 to-purple-600">无人托管</h1>
            <h1 class="text-4xl font-extrabold sm:text-7xl" :class="isNight ? 'font-san' : 'font-en'">Closure Cloud</h1>
            <p class="text-base-content/70 text-2xl mb-8 mt-8 md:mt-4 text-center md:text-left slide-in-bottom delay-50">使用<span class="s-underline">可露希尔云平台</span>，只需点击几下，便可安然入睡。</p>
            <div class="flex-1" />
        ...
        <div class="flex flex-col slide-in-bottom delay-75 lg:ml-6 flex-auto">
            <div class="space-x-4 hidden lg:flex">
                ...
                <div @click="ticketBtnOnClick()" class="bg-base-300 shadow-lg rounded-lg p-2 w-1/2 flex flex-col items-center">
                    ...
                    <div class="w-full items-center flex flex-col border px-2 border-info pt-5 rounded-b-md s-pro">
                        <div class="indicator avatar">
                            <div class="w-16 mask mask-squircle">
                                <img :src="`https://assets.closure.setonink.com/dst/avatar/${k.avatar?.type || 'DEFAULT'}/${k.avatar?.id?.replace('@', '_').replace('#', '_') || 'avatar_activity_GK'}.webp`" alt="斯卡蒂" />
                            </div>
                            <span class="indicator-item indicator-bottom badge badge-info g-glossy text-lg font-en"> Lv{{ k.level }} </span>
                        </div>
