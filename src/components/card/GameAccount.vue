<template>
  <div :class="classes" class="shadow-lg rounded-2xl px-4 py-3 s-pro" v-if="!isAdd">
    <div class="flex items-center">
      ...
      <div class="flex-1" />
      <div class="badge p-3 px-4 badge-info font-zhCN text-lg">{{ game.status?.platform === 1 ? '官' : 'B' }}服</div>
    </div>
    ...
    <div class="grid grid-cols-3">
      <div class="flex flex-col" v-for="m in 3">
        ...
        <span class="text-md font-bold font-en">{{ [game.status?.ap, game.game_config?.map_id || '未选择', game.status?.text][m-1] }}</span>
      </div>
    </div>
    <slot />
  </div>
