        ...
        <div class="bg-base-100 mx-4 px-6 py-4 shadow-lg max-w-md rounded-lg blog">
            ...
            <div v-if="props.screenShots.length > 0 && props.screenShots[0].fileName">
                <div class="carousel w-full flex justify-center">
                    <div v-for="(fileName, index) in props.screenShots[0].fileName" :key="index" class="carousel-item w-full flex justify-center">
                        <div v-show="!imageLoaded[index]" class="loading loading-bars loading-lg"></div>
                        <img v-show="imageLoaded[index]" :src="getBattleScreenShotsLink(props.screenShots[0].host, fileName)" class="w-full" @load="onImageLoad(index)" />
                    </div>
                </div>
                <div class="flex justify-center w-full py-4 gap-2">
                    <a v-for="(_, index) in props.screenShots[0].fileName" :key="index" :href="'#item' + (index + 1)" class="btn btn-outline btn-xs">{{ index + 1 }}</a>
                </div>
...
const dialogId = ref("battleScreenShots");
